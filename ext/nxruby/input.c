...
static VALUE nx_input_key_down(VALUE self, VALUE key_val) {
    ...
    if (key < 0 || key >= SDL_SCANCODE_COUNT) return Qfalse;
...
static VALUE nx_input_pad_down(VALUE self, VALUE btn_val) {
    ...
    if (!input_state.gamepad || btn < 0 || btn >= SDL_GAMEPAD_BUTTON_COUNT) return Qfalse;
...
// --- アナログスティック ---
static VALUE nx_input_pad_lx(VALUE self) { return DBL2NUM(nx_get_pad_axis(SDL_GAMEPAD_AXIS_LEFTX)); }
static VALUE nx_input_pad_ly(VALUE self) { return DBL2NUM(nx_get_pad_axis(SDL_GAMEPAD_AXIS_LEFTY)); }
static VALUE nx_input_pad_rx(VALUE self) { return DBL2NUM(nx_get_pad_axis(SDL_GAMEPAD_AXIS_RIGHTX)); }
static VALUE nx_input_pad_ry(VALUE self) { return DBL2NUM(nx_get_pad_axis(SDL_GAMEPAD_AXIS_RIGHTY)); }
...
static VALUE nx_input_set_mouse_enable(VALUE self, VALUE enable_val) {
    if (RTEST(enable_val)) SDL_ShowCursor(); else SDL_HideCursor();
    return enable_val;
}
